...
<html>
	...
	<?php
		...
		$adminCPassword=$_POST['adminCPassword'];
			...
			// isEmpty field
			if(empty($adminName) || empty($adminEmail) || empty($adminPhone) || empty($adminPassword) || empty($adminCPassword)) {
				echo "<div class='pos'>";
				...
				if($adminPassword==$adminCPassword){
					$query = dbConn()->prepare("UPDATE admin SET adminName='$adminName', adminPhone='$adminPhone', adminPassword='$adminPassword' WHERE adminEmail='".$adminEmail."'");
					// Success
					if($query -> execute()){
						echo "<div class='pos'>";
