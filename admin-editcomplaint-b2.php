	...
	<?php
		...
		$complaintStatus=$_POST['complaintStatus'];
			...
			if(empty($complaintStatus)) {
				...
				$query = dbConn()->prepare("UPDATE complaintsbehalf SET complaintStatus='$complaintStatus' WHERE behalfComplaintID='".$behalfComplaintID."'");
				// Success
				if($query -> execute()){
					echo "<div class='pos'>";
					echo "<img src='icon/icons8-success-64.png'/>";
					echo "<h2>Success!</h2>";
					echo "<p id='valid'>Complaint(On Behalf) is successfully updated.</p>
					<p>Click <a href='admin-complaintlist-onbehalf.php?adminEmail=".$adminEmail."&role=".$role."'><input id='returnBtn' class='button' type='button' name='return' value='Return'></a> to return.</p>
					";
					echo "</div>";
				}
				// Error
