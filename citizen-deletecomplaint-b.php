...
<html>
	...
	<?php
		...
		$query = dbConn()->prepare("UPDATE complaintsbehalf SET complaintStatus='Dropped' WHERE behalfComplaintID='".$behalfComplaintID."'");
		if($query->execute()){
			echo "<div class='pos'>";
			echo "<img src='icon/icons8-success-64.png'/>";
			echo "<h2>Success!</h2>";
			echo "<p id='valid'>Data is successfully deleted.</p>
			<p>Click <a href='citizen-complaintlist-onbehalf.php?citizenIC=".$citizenIC."&role=".$role."'><input id='returnBtn' class='button' type='button' name='return' value='Return'></a> to return.</p>
			";
			echo "</div>";
		}
		else{
