...
<html>
	...
	<?php
		...
		$query = dbConn()->prepare("UPDATE complaints SET complaintStatus='Dropped' WHERE complaintID='".$complaintID."'");
		if($query->execute()){
			echo "<div class='pos'>";
			echo "<img src='icon/icons8-success-64.png'/>";
			echo "<h2>Success!</h2>";
			echo "<p id='valid'>Data is successfully deleted.</p>
			<p>Click <a href='citizen-complaintlist-selfreport.php?citizenIC=".$citizenIC."&role=".$role."'><input id='returnBtn' class='button' type='button' name='return' value='Return'></a> to return.</p>
			";
			echo "</div>";
		}
		else{
